...
class BookService {
    ...
    public async searchBookByTitle(title: string) {
        ...
        if (response.length === 0) return this.responseFactory.responseUser("Error", "Title not found", 404, response);
        return this.responseFactory.responseUser("success", "OK", 200, response);
    }

    public async searchBookByAuthor(author: string) {
        const response = await BookEntity.searchBookByAuthor(author);
        if (response.length === 0) return this.responseFactory.responseUser("Error", "Author not found", 404, response);
        return this.responseFactory.responseUser("success", "OK", 200, response);
    }

    public async getRentedBook(rented: boolean) {
        const response = await BookEntity.getRentedBook(rented);
        if (response.length === 0) return this.responseFactory.responseUser("Error", "not found", 404, response);
        return this.responseFactory.responseUser("success", "OK", 200, response);
    }
    ...
    public async deleteBookById(id: string) {
        const isExistBook = await BookEntity.getBookById(id);
        if (isExistBook) {
            if (isExistBook.isRented === true) return this.responseFactory.responseUser("error", "this book cannot be deleted, it is rented", 400, isExistBook);
    ...
    public async rentedBookById(id: string) {
        const isExistBook = await BookEntity.getBookById(id);
        if (isExistBook) {
            if (isExistBook.isRented === true) return this.responseFactory.responseUser("error", "this book is already rented", 400, isExistBook);
            const dataUpdate = {
                isRented: true,
                updatedAt: new Date()
            }
