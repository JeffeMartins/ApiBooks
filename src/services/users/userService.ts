...
class UserService {
    ...
    public async create(user: IUser) {
        ...
        if (!isExistsUser) {
            ...
            return this.responseFactoryUser.responseUser("success", "User created successfully", 201, user);
        }
        return this.responseFactoryUser.responseUser("success", "User already exists", 409, isExistsUser);
    }
    ...
    public async find(email: string) {
        ...
        if (!response || response === null) return this.responseFactoryUser.responseUser("Error", "User does not exist", 404, {});
